...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['request_id'])) {
    ...
    // Step 2: Delete from inventory_request
    $delete_stmt = $conn->prepare("DELETE FROM inventory_request WHERE id = ?");
    $delete_stmt->bind_param("i", $request_id);
    $delete_stmt->execute();

    // Step 3: Find matching inventory_out records and restore inventory_in
    ...
    while ($out = $result->fetch_assoc()) {
        ...
        // Find any inventory_in with 0 quantity to restore to
        ...
        // Reduce remaining quantity to avoid overrestoring
        $quantity -= $used_qty;
        if ($quantity <= 0) break;
    }
